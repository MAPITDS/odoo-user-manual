document.addEventListener("DOMContentLoaded", function() {
  ...
  if (window.location.pathname !== "/" && window.location.pathname !== "/index.html" && !window.location.pathname.endsWith('/id/') && !window.location.pathname.endsWith('/en/')) {
    ...
    // Buat elemen penampung komentar di bawah artikel dan harus punya akun artikel
    ...
    // Buat elemen penampung komentar di bawah artikel dan tanpa akun/anonimous
    const article = document.querySelector("article");
    if (article) {
      const commentDiv = document.createElement("div");
      commentDiv.id = "disqus_thread";
      commentDiv.style.marginTop = "40px";
      commentDiv.style.paddingTop = "20px";
      commentDiv.style.borderTop = "1px solid var(--md-default-fg-color--lightest)";
      article.appendChild(commentDiv);

      // Konfigurasi Disqus
      window.disqus_config = function () {
        this.page.url = "https://mapitds.github.io/odoo-user-manual" + window.location.pathname;  
        this.page.identifier = window.location.pathname;
      };

      // Load Script Disqus secara dinamis
      const d = document, s = d.createElement('script');
      s.src = 'https://caresys.disqus.com/embed.js'; // <-- Ganti dengan Shortname Anda
